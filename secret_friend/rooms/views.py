...
def invitation_view(request, token, mail=None):
    email = request.GET.get('email', mail)  # Пытаемся получить email как параметр запроса или из URL
    user = User.objects.get(email=email)
    room = get_object_or_404(Room, invitation_token=token)
    if request.user == user:
        room.take_participation(request)
        messages.success(request,'Invitation accepted. Now you are playing this game')
        return redirect('rooms:RoomDetail', pk=room.pk)  # Перенаправьте пользователя куда-нибудь после успешного присоединения
    else:
        messages.error(request, "You are not authenticated. Please login or register")
